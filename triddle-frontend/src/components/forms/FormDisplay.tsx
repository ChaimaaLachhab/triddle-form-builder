...
export function FormDisplay({
  ...
  const [filePreview, setFilePreview] = React.useState<
    Record<string, { name: string; type: string; url?: string }>
  >({});
  const [files, setFiles] = React.useState<Record<string, File>>({});
  const [focusedField, setFocusedField] = React.useState<string | null>(null);
  const [currentFieldIndex, setCurrentFieldIndex] = React.useState(0);
  const [viewMode, setViewMode] = React.useState<"sequential" | "all">(
    "sequential"
  );
  ...
  const handleFileUpload = (fieldId: string, file: File) => {
    // Store the file name as the value in answers
  ...
  const handleNextField = () => {
    ...
    if (
      ...
      (!answers[currentField.fieldId] ||
        (Array.isArray(answers[currentField.fieldId]) &&
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    form.fields.forEach((field) => {
      if (
        ...
        (!answers[field.fieldId] ||
          (Array.isArray(answers[field.fieldId]) &&
            answers[field.fieldId].length === 0))
      ) {
    ...
    if (hasErrors) {
      ...
      if (viewMode === "sequential") {
        const errorIndex = sortedFields.findIndex(
          (field) => newErrors[field.fieldId]
        );
        if (errorIndex !== -1) {
    ...
    const formattedAnswers: Answer[] = Object.entries(answers).map(
      ([fieldId, value]) => {
        // For file fields, include a flag so the backend knows it's a file
        const field = form.fields.find((f) => f.fieldId === fieldId);
        if (field && field.type === "file" && files[fieldId]) {
          return {
            fieldId,
            value,
            file: true, // Add a flag to indicate this is a file field
          };
        }
            ...
            {filePreview[field.fieldId] ? (
              <div className="mt-2 mb-4 p-4 border rounded-lg bg-gray-50 relative">
                <div className="flex items-center">
                  {filePreview[field.fieldId].type.startsWith("image/") &&
                  filePreview[field.fieldId].url ? (
                    <div className="mr-3">
                      <img
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-sm">
                      {filePreview[field.fieldId].name}
                    </p>
                ...
                >
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    <Upload
                      ...
                      style={{
                        color:
                          focusedField === field.fieldId ? primaryColor : "",
                      }}
                    />
                  ...
                  <input
                    type="file"
                    className="hidden"
                    accept={field.fileSettings?.acceptedTypes || "image/*,.pdf"} // Using fileSettings instead of options
                    onFocus={() => handleFocus(field.fieldId)}
                    onBlur={handleBlur}
                    onChange={(e) => {
      ...
      <div className="flex justify-between mt-6">
        <Button
          ...
          style={{
            ...
            color: primaryColor,
            backgroundColor:
              focusedField === sortedFields[currentFieldIndex]?.fieldId
                ? `${primaryColor}22`
                : undefined,
          }}
        >
      ...
      <form onSubmit={handleSubmit}>
        {viewMode === "sequential"
          ? // Sequential view - one field at a time
            sortedFields[currentFieldIndex] &&
            renderField(sortedFields[currentFieldIndex])
          : // All fields view
